...
async function getLyricsFromGenius(url: string) {
    ...
    let lyricsText = lyricsDiv ? lyricsDiv.textContent?.trim() : '';

    if (!lyricsText) {
        const lyricsSections = [...dom.window.document.querySelectorAll("div[class^=Lyrics__Container]").values()].map(x => x.textContent);
        lyricsText = lyricsSections.join("\n");
    }

    // Find the [Intro] section and get everything after it
    const introIndex = lyricsText.indexOf('[Intro]');
    if (introIndex !== -1) {
        lyricsText = lyricsText.substring(introIndex);
    }
